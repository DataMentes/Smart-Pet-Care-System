...
# --- Helper Functions ---
def publish_to_mqtt(topic, payload, retain=True):
    try:
        ...
        result = client.publish(topic, json_payload, retain=retain)
...
def api_status():
    ...
    try:
        supabase_admin.table("users").select("email").limit(1).execute()
...
def signup_verify():
    try:
        ...
        otp_record_res = (
            supabase_admin.table("otp_codes").select("*").eq("email", email).execute()
        )
...
def login():
    try:
        ...
        supabase_admin.table("fcm_tokens").upsert(
            {"email": email, "fcm_token": fcm_token}
        ).execute()
...
def password_reset_confirm():
    try:
        ...
        if is_expired or not is_correct:
            supabase_admin.table("otp_codes").delete().eq("email", email).execute()
            return jsonify({"error": "Invalid or expired OTP. Please start over."}), 401

        users_response = supabase_admin.auth.admin.list_users()
        filtered_users = [
            u.id for u in users_response if u.user_metadata["email"] == email
        ]
    ...
    try:
        ...
        for device in devices_list_res.data:
            ...
            current_status = {
                ...
                "reading": status_res.data if status_res.data else None,
            }
...
def get_device_full_report(device_id):
    try:
        ...
        history_res = (
            supabase_admin.table("pet_food_consumption")
        ...
        if not history_res.data:
            return jsonify({"error": "No data found for this period"}), 200

        food_data = [d["grams"] for d in history_res.data if d.get("grams") is not None]
        total_consumed = sum(food_data)
        days_with_data = len(
            set(
                d["timestamp"].split("T")[0]
                for d in history_res.data
                if d.get("grams") is not None
            )
        )
...
def get_device_schedule(device_id):
    try:
        ...
        feed_time = (
            schedule_data[0]["feed_time"].split(",")
            if schedule_data[0].get("feed_time")
            else []
        )
        amount_grams = (
            schedule_data[0]["amount_grams"].split(",")
            if schedule_data[0].get("amount_grams")
            else []
        )
        schedule_data = [
            {"time": t, "amount": int(a)} for t, a in zip(feed_time, amount_grams)
        ]
    ...
    try:
        ...
        if new_schedule:
            for item in new_schedule:
                time_schedules.append(item.get("time"))
                amount_schedules.append(str(item.get("amount")))

        schedule_to_insert = {
            "device_id": device_id,
            "email": user.email,
            "feed_time": ",".join(time_schedules),
            "amount_grams": ",".join(amount_schedules),
        }
        ...
        topic = f"petfeeder/devices/{device_id}/schedule"
...
@app.route("/api/devices/<string:device_id>/feed-now", methods=["POST"])
def feed_now(device_id):
    jwt = request.headers.get("Authorization").split(" ")[1]
    user = supabase_user.auth.get_user(jwt).user
    if not user:
        return jsonify({"error": "Invalid or expired token"}), 401
    ...
    try:
        ...
        if not amount or not isinstance(amount, int) or amount <= 0:
            return (
                jsonify({"error": "A valid 'amount' (positive integer) is required"}),
                400,
            )

        topic = f"petfeeder/devices/{device_id}/feednow"
        payload = {"grams": amount}

        publish_to_mqtt(topic, payload, retain=False)

        return jsonify(
            {
                "status": "success",
                "message": f"Feed command for {amount}g sent to device {device_id}.",
            }
        )
    except Exception as e:
        return (
